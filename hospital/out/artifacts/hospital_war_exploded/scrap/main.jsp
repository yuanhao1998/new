...
<head>
    <title>资产处置</title>
    <link rel="stylesheet" type="text/css" href="http://localhost:8080/scrap/css/main.css">
    ...
    <script language="JavaScript">
        //打开添加页面
        function insert() {
            window.open("/scrap/insert.jsp?user="+get_user(),"_blank","width=600, height=400,left=660px,top=200px");
        }

        //打开详情页面
        function detail(obj,obj1) {
            //var a=$("#"+obj).attr("id");
            window.open("/scrap/detail.jsp?scrap_no="+obj+"&state="+obj1,"_blank","width=1200, height=800,left=350px,top=100px");
        }
                ...
                if (state == true) {
                    if (groupCheckbox[i].checked) {                //如果处于被选中的状态，执行删除
                        ...
                        $('.data tbody').find("tr").eq(i).each(function () {
                            let trs = $(this).find("td").eq(4).text();
                            if (trs == '已审核') {
                                alert("有单据已审核，请先消审！");
                                state = false;
                            }
...
<body>
<div class="input">
    <form action="/Scrap/select" method="post">
        <table>
            <tr>
                <td>处置日期：<input type="date" name="bdate">至<input type="date" name="edate"></td>
                <td>处置单号：<input type="text" name="scrap_no"></td>
            </tr>
            <tr>
        ...
        <c:forEach items="${result}" var="row" varStatus="status">
            ...
            <c:if test="${row[0]!=null}">
                <tr>
                    ...
                    <td><button id="${row[0]}" onclick="detail(${row[0]},'${row[3]}')">详细</button></td>
                </tr>
            </c:if>
        </c:forEach>
    </table>
